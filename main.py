...
class WebSocketManager:
    ...
    async def disconnect(self, websocket: WebSocket, data=None, inloop=False):
        # 获取客户端索引
        ...
        # 队列为空时直接返回
        if self.event_queue.empty() and not data:
            return
        ...
        if data:
            ...
            try:
                ...
                logging.warning("新的客户端连接失败")
                new_client_id = await self.disconnect(self.connections[new_client_id], inloop=True)
    ...
    async def heartbeat(self, websocket: WebSocket, interval: int = 20):
        while True:
            try:
                logging.info("发送心跳包")
                # logging.debug(f"客户端: {self.get_name(websocket)}")
                # logging.debug(f"客户端ID: {self.get_id(websocket)}")
                # logging.debug(f"客户端连接: {websocket}")
                if websocket not in list(self.connections.values()):
                    logging.info(f"终止{websocket}心跳包")
        ...
        try:
            if websocket not in list(ws_manger.connections.values()):
                ...
                try:
                    ...
                    logging.error(f"终止{websocket}监听进程失败: {e}")
                break
            ...
            if ws_manger.session_bind.get(cache.get("session")) is None:
                ...
                continue
            elif ws_manger.session_bind.get(cache.get("session")) == -1:
                logging.info("客户端id为-1, 移除队列")
                await ws_manger.event_queue.get()
                continue
